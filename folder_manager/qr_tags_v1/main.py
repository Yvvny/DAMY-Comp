...
def _wrap_text_to_max_lines(
    ...
    *,
    ...
    lines = _wrap_text_to_lines(c, text, max_width, font_name=font_name, font_size=font_size)
    ...
    trimmed.append(
        _fit_text_to_width(c, remainder, max_width, font_name=font_name, font_size=font_size)
    )
    return [line for line in trimmed if line]


def _select_wrapped_text_layout(
    c,
    text: str,
    max_width: float,
    max_lines: int,
    *,
    ...
    fallback_font = font_sizes[-1]
    fallback_lines = _wrap_text_to_max_lines(
        c,
        text,
        max_width,
        max_lines,
...
def _render_roster_tag(c, width, height, base_name: str, cell_idx: int, tag_number: int, sheet_name: str, student: StudentRow):
    ...
    draw_fitted_text(c, base_name, text_x, y + PRESET + TEXT_OFFSET_Y * 3, text_max_w, font_size=10)
    draw_fitted_text(c, sheet_name, text_x, y + PRESET + TEXT_OFFSET_Y * 2, text_max_w, font_size=10)

    name_font_size, name_lines = _select_wrapped_text_layout(
        c,
        f"{tag_number}. {student.name}",
        text_max_w,
        3,
        font_name="Helvetica-Bold",
        font_sizes=(12, 11, 10, 9),
    )
    c.setFont("Helvetica-Bold", name_font_size)
    name_y = y + TEXT_OFFSET_Y + PRESET
    line_height = name_font_size + 1
    for idx, line in enumerate(name_lines):
        c.drawString(text_x, name_y - (idx * line_height), line)
...
def generate_qr_tags(
    *,
    ...
    with open(manifest_path, "w", encoding="utf-8") as manifest:
        cells_per_page = NUM_COLUMNS * NUM_ROWS
        cell_idx = 0
        page_has_content = False
        for sheet_index, (sheet_name, rows) in enumerate(per_sheet_render_rows):
            if mode == "roster" and sheet_index > 0 and cell_idx % cells_per_page != 0:
                c.showPage()
                current_output_page += 1
                cell_idx += cells_per_page - (cell_idx % cells_per_page)
                page_has_content = False

            for row in rows:
                if page_has_content and cell_idx % cells_per_page == 0:
                    if mode == "orders":
                ...
                if isinstance(row, tuple):
                    ...
                    if cell_idx % NUM_COLUMNS != 0:
                        ...
                        if cell_idx and cell_idx % cells_per_page == 0:
                            if mode == "orders":
                                _draw_orders_page_counter(c, width, height, current_output_page, total_orders_pages)
                            c.showPage()
                            current_output_page += 1
