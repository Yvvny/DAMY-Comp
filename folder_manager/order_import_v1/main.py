...
try:
    ...
    from folder_manager.db import DB, STAGES, normalize_display_name, parse_job_code, parse_shoot_date_from_display
    from folder_manager.migrations import run_migrations
except Exception:
    DB = None
    run_migrations = None
    STAGES = []
    normalize_display_name = None
    parse_job_code = None
    parse_shoot_date_from_display = None
...
LATE_ORDER_PDF_RE = re.compile(
    r"^(?P<date>\d{6})\s+(?P<pid>P\d{7,})\s+Ordered\s+(?P<count>\d+)-late\.pdf$",
    re.IGNORECASE,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--only-pid",
        default=None,
        help="Only import messages whose parsed Picture Day ID matches this PID.",
    )
    parser.add_argument(
        "--only-item-id",
        type=int,
        default=None,
        help="Only import messages for this workflow item id.",
    )
...
def _build_db_client():
    ...
    name = os.environ.get("DAMY_DB_NAME", str(DB_NAME or "damy_workflow_v2"))
    ...
    try:
        db = DB(host=host, dbname=name, user=user, password=password, port=port)
        if run_migrations is not None:
            run_migrations(db.conninfo)
        return db
...
def _datetime_from_internal_date_ms(internal_date_ms: int) -> datetime | None:
    if internal_date_ms <= 0:
        return None
    try:
        return datetime.fromtimestamp(internal_date_ms / 1000).astimezone()
    except Exception:
        return None


def _parse_shoot_date_from_folder_name(folder_name: str) -> date | None:
    raw = str(folder_name or "").strip()
    if not raw:
        return None
    if callable(parse_shoot_date_from_display) and callable(normalize_display_name):
        try:
            return parse_shoot_date_from_display(normalize_display_name(raw))
        except Exception:
            pass
    match = re.match(r"^\s*(?:\d+\.\s+[^0-9]*)?(\d{6})\b", raw)
    if not match:
        return None
    try:
...
def _late_cutoff_for_shoot_date(shoot_date: date | None, tzinfo) -> datetime | None:
    if shoot_date is None:
        return None
    try:
        return datetime.combine(
            shoot_date - timedelta(days=1),
            datetime_time(18, 30),
            tzinfo=tzinfo,
        )
    except Exception:
...
def _list_folder_candidates_from_db(db, base_dir: Path) -> list[FolderCandidate]:
    ...
    try:
        for item in db.list_all():
            name = str(getattr(item, "disk_name", "") or "").strip()
            if not name:
                continue
            stage = getattr(item, "stage", None)
            stage_value = int(stage) if stage is not None else None
            rows.append((int(getattr(item, "id")), name, stage_value))
    except Exception as exc:
...
def _resolve_target_folder(
    ...
    *,
    ...
    if target_item_id is not None:
        exact_matches = [cand for cand in candidates if cand.item_id == int(target_item_id)]
        if len(exact_matches) == 1:
            chosen = exact_matches[0]
            if chosen.pids and pid not in chosen.pids:
                raise RuntimeError(
                    f"Targeted import PID mismatch for item_id={target_item_id}. "
                    f"email_pid={pid} folder_pids={list(chosen.pids)} folder={chosen.name!r}"
                )
            print(f"[MATCH] targeted_item_match_selected={chosen.name!r} item_id={target_item_id}")
        ...
        if exact_matches:
            raise RuntimeError(
                f"Targeted import found multiple DB candidates for item_id={target_item_id}."
            )
        raise RuntimeError(f"Targeted import could not find DB candidate for item_id={target_item_id}.")
...
def _parse_existing_order_pdf(entry: Path, normalized_pid: str, *, late: bool = False) -> tuple[Path, date, int] | None:
    if not entry.is_file():
        return None
    pattern = LATE_ORDER_PDF_RE if late else ORDER_PDF_RE
...
def _find_existing_order_pdf(
    ...
    *,
    ...
    if preferred_raw:
        ...
        preferred_match = _parse_existing_order_pdf(preferred_entry, normalized_pid, late=late)
        if preferred_match:
    ...
    for entry in folder_path.iterdir():
        parsed = _parse_existing_order_pdf(entry, normalized_pid, late=late)
        if parsed:
...
def _make_order_pdf_name(order_date: date, pid: str, count: int, *, late: bool = False) -> str:
    suffix = "-late" if late else ""
    return f"{order_date:%y%m%d} {pid} Ordered {count}{suffix}.pdf"
...
def _strict_order_pdf_name(
    ...
    *,
    ...
    name = _make_order_pdf_name(order_date, pid, safe_count, late=late)
...
def _append_or_create_order_pdf(
    ...
    *,
    ...
    existing = _find_existing_order_pdf(
        folder_path,
        pid,
        preferred_pdf_path=preferred_existing_pdf_path,
        late=late,
    )
    if not existing:
        name = _strict_order_pdf_name(folder_path, order_date, pid, add_count, late=late)
        target = folder_path / name
    ...
    output_name = _strict_order_pdf_name(folder_path, final_date, pid, next_count, late=late)
...
def _sync_new_folder_to_db(db, folder_name: str) -> int | None:
    if db is None:
        return None
    try:
        return int(db.upsert_from_disk_name(folder_name, 1))
    except Exception as exc:
...
def _process_one_message(
    ...
    *,
    ...
    try:
        ...
        if parsed.pid and db is not None and db.order_import_exists(parsed.pid, parsed.order_no):
            print(
        ...
        if db is not None and db.order_import_exists(effective_pid, parsed.order_no):
            print(
        ...
        gmail_internal_at = _datetime_from_internal_date_ms(internal_date_ms)
        folder_shoot_date = _parse_shoot_date_from_folder_name(target_folder.name)
        previous_db_pdf_path: str | None = None
        previous_db_late_pdf_path: str | None = None
        if db is not None and resolved_item_id is not None:
            try:
                ...
                if db_item:
                    ...
                    previous_db_late_pdf_path = str(getattr(db_item, "late_pdf_path", "") or "").strip() or None
                    folder_shoot_date = getattr(db_item, "shoot_date", None) or folder_shoot_date
            except Exception as exc:
                print(f"[WARN] DB read previous pdf paths failed for id={resolved_item_id}: {exc}")

        late_cutoff_at = _late_cutoff_for_shoot_date(
            folder_shoot_date,
            gmail_internal_at.tzinfo if gmail_internal_at is not None else datetime.now().astimezone().tzinfo,
        )
        is_late = bool(gmail_internal_at is not None and late_cutoff_at is not None and gmail_internal_at > late_cutoff_at)
        if late_cutoff_at is None:
            print(
                f"[LATE] id={msg_id} folder={target_folder.name!r} "
                "cutoff=N/A reason=missing_folder_shoot_date action=NORMAL"
            )
        else:
            gmail_internal_text = (
                f"{gmail_internal_at:%Y-%m-%d %H:%M:%S %Z}"
                if gmail_internal_at is not None
                else "N/A"
            )
            print(
                f"[LATE] id={msg_id} folder_date={folder_shoot_date:%Y-%m-%d} "
                f"cutoff={late_cutoff_at:%Y-%m-%d %H:%M:%S %Z} "
                f"gmail_internal={gmail_internal_text} "
                f"action={'LATE' if is_late else 'NORMAL'}"
            )

        selected_previous_db_pdf_path = previous_db_late_pdf_path if is_late else previous_db_pdf_path

        existing_before = _find_existing_order_pdf(
            target_folder,
            effective_pid,
            preferred_pdf_path=selected_previous_db_pdf_path,
            late=is_late,
        )
        if existing_before:
            old_pdf, old_date, old_count = existing_before
            pdf_source = "folder_scan"
            preferred_raw = str(selected_previous_db_pdf_path or "").strip().strip('"')
            if preferred_raw:
                try:
                    ...
                    pass
            print(
                f"[PDF] id={msg_id} kind={'late' if is_late else 'normal'} "
                f"existing={old_pdf.name!r} count={old_count} "
                f"date={old_date:%Y-%m-%d} source={pdf_source} action=APPEND"
            )
        else:
            print(f"[PDF] id={msg_id} kind={'late' if is_late else 'normal'} existing=None action=CREATE")
        ...
        try:
            ...
            final_pdf = _append_or_create_order_pdf(
                ...
                parsed.order_date,
                temp_pdf_path,
                preferred_existing_pdf_path=selected_previous_db_pdf_path,
            ...
            if db is not None:
                ...
                if resolved_for_pdf is None:
                    try:
                        resolved_row = db.get_item_by_disk_name(target_folder.name)
                        if resolved_row:
                            resolved_for_pdf = int(resolved_row.id)
                            previous_db_pdf_path = resolved_row.pdf_path
                            previous_db_late_pdf_path = str(getattr(resolved_row, "late_pdf_path", "") or "").strip() or None
                ...
                if resolved_for_pdf is not None:
                    try:
                        if is_late:
                            db.set_late_pdf_path(int(resolved_for_pdf), str(final_pdf))
                        else:
                            db.set_pdf_path(int(resolved_for_pdf), str(final_pdf))
                    except Exception as exc:
                        column_name = "late_pdf_path" if is_late else "pdf_path"
                        raise RuntimeError(f"Failed to update DB {column_name} for item {resolved_for_pdf}: {exc}")
                    try:
                        db.record_order_import(
                            ...
                            gmail_internal_at=gmail_internal_at,
        ...
        action = AppliedAction(
            ...
            previous_db_pdf_path=selected_previous_db_pdf_path,
        ...
        try:
            ...
            if db is not None:
                if action.order_history_recorded and action.pid and action.order_no:
                    try:
                        db.delete_order_import_record(action.pid, action.order_no)
                    except Exception as exc:
                ...
                if action.db_inserted_item_id is not None:
                    ...
                    try:
                        if action.pdf_is_late:
                            db.set_late_pdf_path(int(action.item_id), action.previous_db_pdf_path)
                        else:
                            db.set_pdf_path(int(action.item_id), action.previous_db_pdf_path)
                    except Exception as exc:
                        column_name = "late_pdf_path" if action.pdf_is_late else "pdf_path"
                        errors.append(f"{action.message_id}: DB {column_name} rollback failed: {exc}")
        except Exception as exc:
...
def _resolve_only_pid_filter(args: argparse.Namespace, db) -> str | None:
    raw_pid = str(getattr(args, "only_pid", "") or "").strip()
    if raw_pid:
        normalized = _normalize_pid(raw_pid)
        if not normalized:
            raise ValueError(f"Invalid --only-pid value: {raw_pid}")
        return normalized

    only_item_id = getattr(args, "only_item_id", None)
    if only_item_id is None:
        return None
    item = db.get_item_by_id(int(only_item_id)) if db is not None else None
    if item is None:
        raise RuntimeError(f"Workflow item not found for --only-item-id={only_item_id}")

    candidates = [
        str(getattr(item, "pid", "") or ""),
        str(getattr(item, "disk_name", "") or ""),
    ...
    for value in candidates:
        ...
        if normalized:
            return normalized
        if callable(parse_job_code):
            parsed = parse_job_code(value)
            normalized = _normalize_pid(str(parsed or ""))
            if normalized:
                return normalized
    raise RuntimeError(f"Could not resolve PID for --only-item-id={only_item_id}")


def _message_mentions_pid(msg: dict[str, Any] | None, pid: str) -> bool:
    if not msg:
        return False
    ...
    digits = normalized[1:]
    payload = msg.get("payload", {}) or {}
    values = [str(msg.get("snippet", "") or "")]
    for header_name in ("Subject", "From", "To"):
        values.append(_extract_header(payload, header_name))
    plain_parts: list[str] = []
    html_parts: list[str] = []
    _collect_message_bodies(payload, plain_parts, html_parts)
    values.extend(plain_parts)
    values.extend(html_parts)
    text = "\n".join(values).upper()
    compact = re.sub(r"[^A-Z0-9]", "", text)
    return normalized.upper() in text or normalized.upper() in compact or digits in compact


def _filter_message_ids_for_pid(service, message_ids: list[str], only_pid: str) -> list[str]:
    normalized_target = _normalize_pid(only_pid)
    ...
    for msg_id in message_ids:
        ...
        try:
            msg = service.users().messages().get(userId="me", id=msg_id, format="full").execute()
            parsed = _parse_message_order(msg)
            msg_pid = _normalize_pid(str(parsed.pid or ""))
            if msg_pid == normalized_target:
                selected.append(msg_id)
        except Exception as exc:
            if _message_mentions_pid(msg, normalized_target):
                raise RuntimeError(
                    f"Message {msg_id} appears to belong to PID {normalized_target}, "
                    f"but could not be parsed: {exc}"
                ) from exc
            skipped_parse += 1
            print(f"[FILTER] message={msg_id} skipped while filtering for pid={normalized_target}: {exc}")
    print(
        f"[FILTER] only_pid={normalized_target} selected={len(selected)} "
        f"from={len(message_ids)} parse_skipped={skipped_parse}"
    )
    return selected
...
def run(args: argparse.Namespace) -> int:
    ...
    db = _build_db_client()
    only_pid = _resolve_only_pid_filter(args, db)
    if only_pid:
        ordered_message_ids = _filter_message_ids_for_pid(service, ordered_message_ids, only_pid)
    summary = {
    ...
    for idx, msg_id in enumerate(ordered_message_ids, start=1):
        ...
        try:
            ok, detail, folder_name, folder_action, item_id, action = _process_one_message(
                ...
                is_cancelled=is_cancelled,
                db=db,
                target_item_id=getattr(args, "only_item_id", None),
            )
            if ok:
