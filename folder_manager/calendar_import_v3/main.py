...
def _sync_db_note_for_folder(db, disk_name: str, note_text: str) -> None:
    stage = detect_stage_from_disk_name(disk_name)
    ...
    item_id = int(db.upsert_from_disk_name(disk_name, stage))
...
def load_existing_calendar_keys_from_db(db) -> set[tuple[str, str]]:
    ...
    try:
        for item in db.list_all():
            source_name = getattr(item, "display_name", None) or getattr(item, "disk_name", None) or ""
...
def load_existing_pids_from_db(db) -> set[str]:
    ...
    try:
        for item in db.list_all():
            ...
            for token in re.findall(r"\bP\d{8,}\b", str(disk_name or "")):
                pids.add(token)
            for token in re.findall(r"\bP\d{8,}\b", str(display_name or "")):
                pids.add(token)
...
def handle_missing_school_actions(
    ...
    for row in missing_rows:
        ...
        if action == "reschedule":
            ...
            try:
                current_item = db.get_item_by_disk_name(folder_name)
                target_item = db.get_item_by_disk_name(target_name)
