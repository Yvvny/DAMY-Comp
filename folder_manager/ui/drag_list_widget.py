...
from ..db import WORKFLOW_DOMAIN_PREPAID
...
class DragListWidget(QListWidget):
    ...
    def _iter_source_items(source) -> list[QListWidgetItem]:
        ...
        if not selected_items and hasattr(source, "currentItem"):
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        source_stage = getattr(source, "stage", None)
        old_stage = int(source_stage) if source_stage is not None else -1
        source_requires_flags = bool(getattr(source, "requires_flags_to_leave", False))
        source_day_name = str(getattr(source, "day_name", "source"))
        new_stage = self.stage

        move_rows: list[tuple[Optional[QListWidgetItem], int, str]] = []
        for item, item_id, disk_name in source_rows:
            if (item_id is None) and disk_name:
                try:
                    db_item = self.db.get_item_by_disk_name(str(disk_name))
        ...
        if source_requires_flags and not self.requires_flags_to_leave:
            blocked_names = []
        ...
        moved: list[tuple[Optional[QListWidgetItem], int, str]] = []
        errors: list[str] = []
        source_domain = (getattr(source, "workflow_domain", "") or "").strip().lower()
        for item, item_id, disk_name in move_rows:
            try:
                # UI drag/drop updates DB stage only. Disk folders are unchanged.
                if source_domain and source_domain != WORKFLOW_DOMAIN_PREPAID:
        ...
        for item, item_id, _ in sorted(
            (row for row in moved if row[0] is not None),
            key=lambda x: source.row(x[0]),
            reverse=True,
        ):
