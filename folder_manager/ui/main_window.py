...
from ..proof_sorter import sort_folders
...
FOLDER_ASSET_HINT_TEXT = (
    ...
    "Excel: A=name, B=class, C=password (blank class uses sheet name)."
)
...
class DragDropFolders(QWidget):
    def __init__(
        ...
        self.folder_action_popup_mode_stage: int | None = None
        ...
        self._folder_action_pdf_click_guard_until: float = 0.0
        self._folder_action_toggle_suppress_key: tuple[int, int] | None = None
        self.folder_action_inline_warning_label: QLabel | None = None
        self.folder_action_stage2_section: QWidget | None = None
        self.folder_action_stage2_sort_button: QPushButton | None = None
        self.folder_action_stage3_section: QWidget | None = None
        self.folder_action_stage3_splitter: QSplitter | None = None
        self.folder_action_stage3_bulk_button: QPushButton | None = None
        self.folder_action_stage3_pdf_button: QPushButton | None = None
        ...
        self._restoring_workspace_sizes = False
        self.current_workspace: str = "prepaid"
        self.workspace_stack: QStackedWidget | None = None
        self.workspace_buttons: dict[str, QPushButton] = {}
        self._workspace_nav_top_layout: QVBoxLayout | None = None
        self._workspace_nav_bottom_layout: QVBoxLayout | None = None
        self.upcoming_column_widget: QWidget | None = None
        self.prepaid_page: QWidget | None = None
        self.proofing_page: QWidget | None = None
        self.yearbook_page: QWidget | None = None
        self._proofing_host_layout: QVBoxLayout | None = None
        self._proofing_loading_label: QLabel | None = None
        self._proofing_widget: QWidget | None = None
        self._proofing_retry_timer = QTimer(self)
        self._proofing_retry_timer.setSingleShot(True)
        self._proofing_retry_timer.timeout.connect(self._attempt_proofing_embed_load)
        self._proofing_retry_interval_ms = 1200
        self._proofing_embed_base_dir = (os.environ.get("DAMY_PROOF_BASE_DIR") or self.base_dir or r"T:\DAMY").strip() or (self.base_dir or r"T:\DAMY")
        self._yearbook_host_layout: QVBoxLayout | None = None
        self._yearbook_loading_label: QLabel | None = None
        self._yearbook_widget: QWidget | None = None
        self._yearbook_retry_timer = QTimer(self)
        self._yearbook_retry_timer.setSingleShot(True)
        self._yearbook_retry_timer.timeout.connect(self._attempt_yearbook_embed_load)
        self._yearbook_retry_interval_ms = 1200
        self._yearbook_embed_base_dir = (os.environ.get("DAMY_YEARBOOK_BASE_DIR") or self.base_dir or r"T:\DAMY").strip() or (self.base_dir or r"T:\DAMY")
    ...
    def closeEvent(self, event):  # type: ignore[override]
        ...
        try:
            ...
            self._save_workspace_splitter_sizes()
        except Exception:
            pass
        try:
            self._proofing_retry_timer.stop()
        except Exception:
            pass
        try:
            self._yearbook_retry_timer.stop()
        except Exception:
            pass
        app = QApplication.instance()
        if app is not None:
    ...
    def _save_workspace_splitter_sizes(self) -> None:
        ...
        if splitter is None or self._restoring_workspace_sizes:
            return
        sizes = [int(size) for size in splitter.sizes()]
        if len(sizes) != 2 or any(size <= 0 for size in sizes):
            return
        self._column_size_settings().setValue("ui/workspace_splitter_sizes", json.dumps(sizes))

    def _restore_workspace_splitter_sizes(self) -> None:
        splitter = self.workspace_splitter
        if splitter is None:
            return
        raw_value = self._column_size_settings().value("ui/workspace_splitter_sizes", "")
        if not raw_value:
            return
        try:
            parsed = json.loads(str(raw_value))
            sizes = [max(120, int(value)) for value in parsed]
        except Exception:
            return
        if len(sizes) != 2:
            return

        self._restoring_workspace_sizes = True
        try:
            splitter.setSizes(sizes)
        finally:
            self._restoring_workspace_sizes = False

    def _create_workspace_nav_button(self, text: str, tooltip: str, target: str) -> QPushButton:
        button = self._create_stage_action_button(
            text,
            tooltip,
            lambda _checked=False, t=target: self._switch_workspace(t),
        )
    ...
    def _apply_workspace_button_styles(self, active_target: str) -> None:
        for target, button in self.workspace_buttons.items():
            is_active = (target == active_target)
            button.setChecked(is_active)
            if is_active:
                button.setStyleSheet(
                    "QPushButton {"
                    "font-size: 20px; font-weight: 800; color: #f4f8ff; "
                    "border: 1px solid #5f80a0; border-radius: 6px; padding: 8px 10px; "
                    "background-color: #324356; }"
                )
            else:
                button.setStyleSheet(
                    "QPushButton {"
                    "font-size: 20px; font-weight: 700; color: #d7e1ed; "
                    "border: 1px solid #4a5b6d; border-radius: 6px; padding: 8px 10px; "
                    "background-color: #2a3441; }"
                    "QPushButton:hover { background-color: #344253; }"
                )

    def _clear_nav_layout(self, layout: QVBoxLayout | QHBoxLayout | None) -> None:
        if layout is None:
            return
        while layout.count():
            item = layout.takeAt(0)
            ...
            if widget is not None:
                ...
                continue
            if child_layout is not None:
                self._clear_nav_layout(child_layout)  # type: ignore[arg-type]

    def _arrange_workspace_nav_buttons(self, active_target: str) -> None:
        top_layout = self._workspace_nav_top_layout
        bottom_layout = self._workspace_nav_bottom_layout
        if top_layout is None or bottom_layout is None:
            return

        for button in self.workspace_buttons.values():
            button.hide()

        self._clear_nav_layout(top_layout)
        self._clear_nav_layout(bottom_layout)

        placement_map: dict[str, dict[str, list[str]]] = {
            "prepaid": {"top": [], "bottom": ["proofing", "yearbook"]},
            "proofing": {"top": ["prepaid"], "bottom": ["yearbook"]},
            "yearbook": {"top": ["prepaid", "proofing"], "bottom": []},
        }
        placement = placement_map.get(active_target, {"top": [], "bottom": []})

        for target in placement.get("top", []):
            button = self.workspace_buttons.get(target)
            if button is None:
                continue
            top_layout.addWidget(button)
            button.show()

        for target in placement.get("bottom", []):
            button = self.workspace_buttons.get(target)
            if button is None:
                continue
    ...
    def _switch_workspace(self, target: str) -> None:
        stack = self.workspace_stack
        if stack is None:
            return
        index_map = {"prepaid": 0, "proofing": 1, "yearbook": 2}
        if target not in index_map:
            return
        target_index = int(index_map[target])
        if target_index >= stack.count():
            return

        self.current_workspace = target
        stack.setCurrentIndex(target_index)
        self._arrange_workspace_nav_buttons(target)
        self._apply_workspace_button_styles(target)
        if target == "proofing":
            self._attempt_proofing_embed_load()
        if target == "yearbook":
            self._attempt_yearbook_embed_load()
        self._sync_global_search_to_workspace(target)

    def _workspace_display_name(self, target: str) -> str:
    ...
    def _active_workspace_widget(self, target: str | None = None):
        key = (target or self.current_workspace or "prepaid").strip().lower()
        if key == "proofing":
            return self._proofing_widget
        if key == "yearbook":
            return self._yearbook_widget
        return None

    def _embedded_workspace_busy_state(self, target: str) -> tuple[bool, str]:
        key = (target or "").strip().lower()
        if key not in {"proofing", "yearbook"}:
            return False, ""
        widget = self._active_workspace_widget(key)
        if widget is None:
            return False, ""
        try:
            if hasattr(widget, "is_busy"):
                result = widget.is_busy()
                if isinstance(result, tuple) and result:
                    busy = bool(result[0])
                    label = str(result[1]) if len(result) > 1 else ""
                    return busy, label
                return bool(result), ""
        except Exception:
            pass

        # Fallback for embedded widgets that do not expose is_busy.
        if key == "proofing":
            thread = getattr(widget, "photodeck_thread", None)
            if thread is not None:
                try:
                    ...
                    pass
        if key == "yearbook":
            proc = getattr(widget, "yearbook_import_process", None)
            try:
    ...
    def _sync_global_search_to_workspace(self, target: str | None = None) -> None:
        key = (target or self.current_workspace or "prepaid").strip().lower()
        text = self.search_input.text() if self.search_input is not None else ""
        if key == "prepaid":
            # Re-apply to prepaid board to avoid stale visibility when switching back.
            self._perform_prepaid_search(text)
            return
        self._perform_upcoming_only_search(text)
        self._perform_embedded_workspace_search(key, text)

    def _perform_upcoming_only_search(self, text: str) -> None:
        if self.upcoming_column_widget is None:
            return
        upcoming_list = None
        for col_widget, lw in self.column_widgets:
            if col_widget is self.upcoming_column_widget:
                upcoming_list = lw
                break
        if upcoming_list is None:
            return
        needle = (text or "").strip().lower()
        for i in range(upcoming_list.count()):
            row_item = upcoming_list.item(i)
            if row_item is None:
                continue
            progress_text = str(row_item.data(Qt.ItemDataRole.UserRole + 2) or "").lower()
            match = (not needle) or (needle in row_item.text().lower()) or (needle in progress_text)
            row_item.setHidden(not match)
        self.upcoming_column_widget.setVisible(True)

    def _perform_embedded_workspace_search(self, target: str, text: str) -> None:
        widget = self._active_workspace_widget(target)
        if widget is None:
            return
        try:
            if hasattr(widget, "set_embedded_mode"):
                widget.set_embedded_mode(True)
            embedded_search = getattr(widget, "search_input", None)
            if embedded_search is not None:
                embedded_search.hide()
            if hasattr(widget, "set_external_search_text"):
                widget.set_external_search_text(text)
            elif hasattr(widget, "perform_search"):
                widget.perform_search(text)
        except Exception as exc:
            _append_ui_runtime_log(
                f"embedded search sync failed workspace={target} err={exc}"
            )

    def _attempt_proofing_embed_load(self) -> None:
        if self._proofing_widget is not None:
            self._proofing_retry_timer.stop()
            if self._proofing_loading_label is not None:
                self._proofing_loading_label.hide()
            return

        host_layout = self._proofing_host_layout
        if host_layout is None:
            return

        loading_label = self._proofing_loading_label
        if loading_label is not None:
            loading_label.setText("Loading DAMY ONLINE Proofing...")
            loading_label.show()

        try:
            module = importlib.import_module("folder_manager.proofing_online.ui.views.main_window")
            widget_cls = getattr(module, "DragDropFolders", None)
            if widget_cls is None:
                raise RuntimeError("Proofing embed class DragDropFolders was not found.")

            widget = widget_cls(
                self._proofing_embed_base_dir,
                db_host=self.db_host,
                dbname=self.dbname,
                user=self.db_user,
                password=self.db_password,
                port=self.db_port,
                workflow_domain=WORKFLOW_DOMAIN_PROOFING,
            )
            widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            host_layout.addWidget(widget, 1)
            self._proofing_widget = widget
            try:
                if hasattr(widget, "set_embedded_mode"):
                    widget.set_embedded_mode(True)
                embedded_search = getattr(widget, "search_input", None)
                ...
                pass
            self._sync_global_search_to_workspace("proofing")
            self._proofing_retry_timer.stop()
            if loading_label is not None:
                loading_label.hide()
        except Exception as exc:
            _append_ui_runtime_log(f"proofing embed load failed; retrying silently: {exc}")
            self._proofing_retry_timer.start(max(500, int(self._proofing_retry_interval_ms)))

    def _attempt_yearbook_embed_load(self) -> None:
        if self._yearbook_widget is not None:
            self._yearbook_retry_timer.stop()
            if self._yearbook_loading_label is not None:
                self._yearbook_loading_label.hide()
            return

        host_layout = self._yearbook_host_layout
        if host_layout is None:
            return

        loading_label = self._yearbook_loading_label
        if loading_label is not None:
            loading_label.setText("Loading DAMY Yearbook...")
            loading_label.show()

        try:
            module = importlib.import_module("folder_manager.yearbook_online.ui.views.main_window")
            widget_cls = getattr(module, "DragDropFolders", None)
            if widget_cls is None:
                raise RuntimeError("Yearbook embed class DragDropFolders was not found.")

            widget = widget_cls(
                self._yearbook_embed_base_dir,
                db_host=self.db_host,
                dbname=self.dbname,
                user=self.db_user,
                password=self.db_password,
                port=self.db_port,
                workflow_domain=WORKFLOW_DOMAIN_YEARBOOK,
            )
            widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
            host_layout.addWidget(widget, 1)
            self._yearbook_widget = widget
            try:
                if hasattr(widget, "set_embedded_mode"):
                    widget.set_embedded_mode(True)
                embedded_search = getattr(widget, "search_input", None)
                ...
                pass
            self._sync_global_search_to_workspace("yearbook")
            self._yearbook_retry_timer.stop()
            if loading_label is not None:
                loading_label.hide()
        except Exception as exc:
            _append_ui_runtime_log(f"yearbook embed load failed; retrying silently: {exc}")
            self._yearbook_retry_timer.start(max(500, int(self._yearbook_retry_interval_ms)))
    ...
    def initialize_ui(self):
        ...
        self._workspace_nav_top_layout = None
        self._workspace_nav_bottom_layout = None
        self.upcoming_column_widget = None
        self.prepaid_page = None
        self.proofing_page = None
        self.yearbook_page = None
        self._proofing_host_layout = None
        self._proofing_loading_label = None
        self._proofing_widget = None
        self._yearbook_host_layout = None
        self._yearbook_loading_label = None
        self._yearbook_widget = None
        ...
        self.refresh_button = btn_refresh
        toolbar_row.addWidget(btn_refresh)
        self.main_layout.addLayout(toolbar_row)

        refresh_help = QLabel(
            "Refresh applies to the active workspace: full precheck for Prepaid, domain-specific refresh for Proofing/YearBook."
        )
        ...
        self.workspace_splitter.splitterMoved.connect(
            lambda _pos, _index: self._save_workspace_splitter_sizes()
        )
        ...
        self.workspace_stack.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)

        self.prepaid_page = QWidget()
        prepaid_layout = QVBoxLayout(self.prepaid_page)
        prepaid_layout.setContentsMargins(0, 0, 0, 0)
        prepaid_layout.setSpacing(0)
        ...
        prepaid_layout.addWidget(self.column_splitter)

        self.proofing_page = QWidget()
        proofing_layout = QVBoxLayout(self.proofing_page)
        proofing_layout.setContentsMargins(0, 0, 0, 0)
        proofing_layout.setSpacing(8)
        self._proofing_loading_label = QLabel("Loading DAMY ONLINE Proofing...")
        self._proofing_loading_label.setAlignment(Qt.AlignCenter)
        self._proofing_loading_label.setStyleSheet(
            "QLabel { color: #c6d2e0; font-size: 14px; font-weight: 600; padding: 8px; }"
        )
        proofing_layout.addWidget(self._proofing_loading_label)
        proofing_host = QWidget()
        self._proofing_host_layout = QVBoxLayout(proofing_host)
        self._proofing_host_layout.setContentsMargins(0, 0, 0, 0)
        self._proofing_host_layout.setSpacing(0)
        proofing_layout.addWidget(proofing_host, 1)

        self.yearbook_page = QWidget()
        yearbook_layout = QVBoxLayout(self.yearbook_page)
        yearbook_layout.setContentsMargins(0, 0, 0, 0)
        yearbook_layout.setSpacing(8)
        self._yearbook_loading_label = QLabel("Loading DAMY Yearbook...")
        self._yearbook_loading_label.setAlignment(Qt.AlignCenter)
        self._yearbook_loading_label.setStyleSheet(
            "QLabel { color: #c6d2e0; font-size: 14px; font-weight: 600; padding: 8px; }"
        )
        yearbook_layout.addWidget(self._yearbook_loading_label)
        yearbook_host = QWidget()
        self._yearbook_host_layout = QVBoxLayout(yearbook_host)
        self._yearbook_host_layout.setContentsMargins(0, 0, 0, 0)
        self._yearbook_host_layout.setSpacing(0)
        yearbook_layout.addWidget(yearbook_host, 1)

        self.workspace_stack.addWidget(self.prepaid_page)
        self.workspace_stack.addWidget(self.proofing_page)
        self.workspace_stack.addWidget(self.yearbook_page)
        top_nav_host = QWidget()
        self._workspace_nav_top_layout = QVBoxLayout(top_nav_host)
        self._workspace_nav_top_layout.setContentsMargins(0, 0, 0, 0)
        self._workspace_nav_top_layout.setSpacing(10)

        bottom_nav_host = QWidget()
        self._workspace_nav_bottom_layout = QVBoxLayout(bottom_nav_host)
        self._workspace_nav_bottom_layout.setContentsMargins(0, 0, 0, 0)
        self._workspace_nav_bottom_layout.setSpacing(10)

        right_layout.addWidget(top_nav_host)
        right_layout.addWidget(self.workspace_stack, 1)
        right_layout.addWidget(bottom_nav_host)

        btn_prepaid = self._create_workspace_nav_button(
            "Prepaid",
            "Show DAMY-Comp stage board (2-7) on the right.",
            "prepaid",
        )
        btn_proofing = self._create_workspace_nav_button(
            "Proofing",
            "Load DAMY ONLINE proofing workflow on the right side.",
            "proofing",
        )
        btn_yearbook = self._create_workspace_nav_button(
            "YearBook",
            "Load DAMY Yearbook workflow on the right side.",
            "yearbook",
        )
        self.workspace_buttons = {
            "prepaid": btn_prepaid,
            "proofing": btn_proofing,
            "yearbook": btn_yearbook,
        }
        ...
        for stage_def in STAGES:
            ...
            if stage_def.edit_column:
                ...
                if int(stage) == 1:
                    self.upcoming_column_widget = column
                else:
        ...
        if self.upcoming_column_widget is not None:
            self.workspace_splitter.addWidget(self.upcoming_column_widget)
        else:
            missing_upcoming = QLabel("1. Upcoming column not available")
            missing_upcoming.setAlignment(Qt.AlignCenter)
            missing_upcoming.setStyleSheet("QLabel { color: #ffcc80; font-weight: 700; }")
            self.workspace_splitter.addWidget(missing_upcoming)
        self.workspace_splitter.addWidget(right_panel)
        self.workspace_splitter.setSizes([320, 1220])
        ...
        target_workspace = self.current_workspace if self.current_workspace in {"prepaid", "proofing", "yearbook"} else "prepaid"
    ...
    def _poll_order_import_process(self):
        ...
        if exit_code == 0:
            ...
            if fail_count > 0:
                ...
                self._show_info_text_dialog("Order Import Failure Details", detail_text)
    ...
    def _format_order_import_failure_details(self, details: list[dict]) -> str:
        ...
        for item in details:
            ...
            if "pid not found in email" in reason_lower or "pid not found in email" in detail_lower:
            ...
            if "manual folder selection was skipped" in detail_lower:
                what_happened = "A folder selection window was shown, but no folder was selected."
            elif "stream has ended unexpectedly" in detail_lower:
                what_happened = (
            ...
            if detail:
                lines.append(f"Technical detail: {detail}")
            blocks.append("\n".join(lines))

        if not blocks:
            return "This email was not imported."
        return "\n\n".join(blocks[:200])
    ...
    def _apply_order_import_summary_markers(self, summary: dict) -> int:
        ...
        def _resolve_item_id(folder_name: str) -> int | None:
            ...
            try:
                item = self.db.get_item_by_disk_name(
                    name,
    ...
    def _refresh_operation_status_ui(self) -> None:
        if not getattr(self, "operation_status_label", None):
            return
        running = [label for label in getattr(self, "_active_operations", {}).values() if str(label).strip()]
        self._set_operation_buttons_enabled(not bool(running))
        retry_text = (getattr(self, "_retry_status_text", "") or "").strip()
        completion_text = (getattr(self, "_completion_status_text", "") or "").strip()
        if running:
    ...
    def _set_operation_buttons_enabled(self, enabled: bool) -> None:
        if getattr(self, "import_calendar_button", None):
            self.import_calendar_button.setEnabled(enabled)
        if getattr(self, "import_order_button", None):
    ...
    def _asset_allowed_suffixes(self, asset_kind: str) -> set[str]:
        ...
        if kind in {"pdf", "pdf_2", "pdf_3", "pdf_4", "qr_roster", "qr_orders"}:
            return {".pdf"}
        if kind == "excel":
    ...
    def _run_cancellation_in_ui(self) -> bool:
        ...
        def _snapshot_item(item):
            ...
            return {
                ...
                "workflow_domain": str(getattr(item, "workflow_domain", "") or WORKFLOW_DOMAIN_PREPAID),
            ...
            while True:
                ...
                if action == "cancel":
                    ...
                    if not confirm_cancel:
                        continue
                    try:
                        source_item = self.db.get_item_by_disk_name(
                            folder_name,
                            domain=WORKFLOW_DOMAIN_PREPAID,
                            stage=1,
                        )
                        ...
                        self._delete_db_item_by_disk_name(
                            folder_name,
                            domain=WORKFLOW_DOMAIN_PREPAID,
                            stage=1,
                        )
                ...
                try:
                    ...
                    target_item = self.db.get_item_by_disk_name(
                        target_name,
                        domain=WORKFLOW_DOMAIN_PREPAID,
                        stage=1,
                    )
    ...
    def _ordered_reschedule_candidates(self, selectable_folder_map: dict[str, dict]) -> list[str]:
        ...
        try:
            for it in self.db.list_by_stage(1, domain=WORKFLOW_DOMAIN_PREPAID):
    ...
    def _ensure_folder_action_popup(self) -> None:
        ...
        panel.setStyleSheet(
            ...
            "QFrame#folderActionStage3Panel {"
            " background: #243244; border: 1px solid #4a607c; border-radius: 10px; }"
        )
        ...
        stage2_section = QWidget()
        stage2_layout = QVBoxLayout(stage2_section)
        stage2_layout.setContentsMargins(0, 0, 0, 0)
        stage2_layout.setSpacing(8)
        stage2_title = QLabel("2. Sort")
        stage2_title.setStyleSheet("QLabel { color: #e4ecf7; font-size: 12px; font-weight: 700; }")
        stage2_button = QPushButton("Run Sort")
        stage2_button.setToolTip("Run sorter for selected folder in 2. Sort")
        stage2_button.clicked.connect(self._run_stage2_sort_from_asset)
        stage2_layout.addWidget(stage2_title)
        stage2_layout.addWidget(stage2_button)
        stage2_section.hide()
        layout.addWidget(stage2_section)
        ...
        stage3_title = QLabel("3. Photodeck Upload/PDF Packets")
        stage3_title.setStyleSheet("QLabel { color: #e4ecf7; font-size: 12px; font-weight: 700; }")
        stage3_layout.addWidget(stage3_title)

        stage3_splitter = QSplitter(Qt.Orientation.Horizontal)
        stage3_splitter.setChildrenCollapsible(False)
        stage3_splitter.setHandleWidth(10)
        stage3_splitter.setStyleSheet("QSplitter::handle { background: #4f6582; }")

        bulk_panel = QFrame()
        bulk_panel.setObjectName("folderActionStage3Panel")
        bulk_layout = QVBoxLayout(bulk_panel)
        bulk_layout.setContentsMargins(10, 10, 10, 10)
        bulk_layout.setSpacing(8)
        bulk_label = QLabel("Bulk Upload")
        bulk_label.setStyleSheet("QLabel { color: #eef3f9; font-size: 12px; font-weight: 700; }")
        bulk_button = QPushButton("Bulk Upload")
        bulk_button.clicked.connect(self._run_stage3_bulk_upload_from_asset)
        bulk_layout.addWidget(bulk_label)
        bulk_layout.addWidget(bulk_button)
        bulk_layout.addStretch(1)

        pdf_panel = QFrame()
        pdf_panel.setObjectName("folderActionStage3Panel")
        pdf_layout = QVBoxLayout(pdf_panel)
        pdf_layout.setContentsMargins(10, 10, 10, 10)
        pdf_layout.setSpacing(8)
        pdf_label = QLabel("PDF")
        pdf_label.setStyleSheet("QLabel { color: #eef3f9; font-size: 12px; font-weight: 700; }")
        pdf_button = QPushButton("PDF")
        pdf_button.clicked.connect(self._run_stage3_pdf_from_asset)
        pdf_layout.addWidget(pdf_label)
        pdf_layout.addWidget(pdf_button)
        pdf_layout.addStretch(1)

        stage3_splitter.addWidget(bulk_panel)
        stage3_splitter.addWidget(pdf_panel)
        stage3_splitter.setSizes([260, 260])
        stage3_layout.addWidget(stage3_splitter)
        stage3_section.hide()
        ...
        qr_roster_zone.doubleClicked.connect(self._on_folder_action_qr_roster_open_requested)
        ...
        qr_orders_zone.doubleClicked.connect(self._on_folder_action_qr_orders_open_requested)
        ...
        self.folder_action_stage2_section = stage2_section
        self.folder_action_stage2_sort_button = stage2_button
        self.folder_action_stage3_section = stage3_section
        self.folder_action_stage3_splitter = stage3_splitter
        self.folder_action_stage3_bulk_button = bulk_button
        self.folder_action_stage3_pdf_button = pdf_button
    ...
    def _on_folder_action_selection_changed(self, lw: DragListWidget) -> None:
        if self.folder_action_anchor_list is lw and not lw.selectedItems():
            panel = self.folder_action_popup
            # DragListWidget clears selection on mouse press before itemClicked.
            # Keep popup state until click handler decides toggle/open.
            if panel is not None and panel.isVisible():
                return
            self._hide_folder_action_popup()
            return
        if self.folder_action_anchor_list is lw and self.folder_action_popup is not None and self.folder_action_popup.isVisible():
            stage_value = self._resolve_folder_action_stage(lw)
            selected_items = [it for it in lw.selectedItems() if it is not None]
            selected_count = len(selected_items)
            if selected_items:
                first = selected_items[0]
                disk_name = str(first.data(ROLE_DISK_NAME) or first.text() or "").strip()
                if disk_name:
                    self.folder_action_anchor_disk_name = disk_name
                    self._set_folder_action_popup_mode(stage_value, disk_name, selected_count=selected_count)
            self._resize_folder_action_popup_host()

    def _clear_folder_action_toggle_suppress_key(self, expected_key: tuple[int, int]) -> None:
        if self._folder_action_toggle_suppress_key == expected_key:
            self._folder_action_toggle_suppress_key = None

    def _resolve_folder_action_stage(self, lw: DragListWidget | None = None) -> int:
        source = lw if lw is not None else self.folder_action_anchor_list
        if source is None:
            return 1
        try:
            return int(getattr(source, "stage", 1) or 1)
        except Exception:
            return 1

    def _folder_action_selected_items_for_stage(self, expected_stage: int) -> list[QListWidgetItem]:
        lw = self.folder_action_anchor_list
        if lw is None:
            return []
        if self._resolve_folder_action_stage(lw) != int(expected_stage):
            return []
        selected = [it for it in lw.selectedItems() if it is not None]
        if selected:
            return selected
        if self.folder_action_anchor_item_id is not None:
            fallback = self._find_item_by_id_in_list(lw, int(self.folder_action_anchor_item_id))
            if fallback is not None:
                return [fallback]
        return []

    def _folder_action_selected_disk_names_for_stage(self, expected_stage: int) -> list[str]:
        names: list[str] = []
        seen: set[str] = set()
        for item in self._folder_action_selected_items_for_stage(expected_stage):
            disk_name = str(item.data(ROLE_DISK_NAME) or item.text() or "").strip()
            if not disk_name or disk_name in seen:
                continue
            seen.add(disk_name)
            names.append(disk_name)
        return names

    def _set_folder_action_popup_mode(self, stage_value: int, disk_name: str, *, selected_count: int = 1) -> None:
        title_label = getattr(self, "folder_action_title_label", None)
        hint_label = getattr(self, "folder_action_hint_label", None)

        stage1_widgets = [
            getattr(self, "folder_action_make_orders_zone", None),
            getattr(self, "folder_action_make_orders_zone_2", None),
            getattr(self, "folder_action_make_orders_add_button", None),
            getattr(self, "folder_action_pdf_zone", None),
            getattr(self, "folder_action_pdf_zone_2", None),
            getattr(self, "folder_action_pdf_zone_3", None),
            getattr(self, "folder_action_pdf_zone_4", None),
            getattr(self, "folder_action_pdf_add_button", None),
            getattr(self, "folder_action_excel_zone", None),
            getattr(self, "folder_action_qr_roster_zone", None),
            getattr(self, "folder_action_qr_orders_zone", None),
            getattr(self, "folder_action_inline_warning_label", None),
        ]
        stage2_section = getattr(self, "folder_action_stage2_section", None)
        stage2_button = getattr(self, "folder_action_stage2_sort_button", None)
        stage3_section = getattr(self, "folder_action_stage3_section", None)

        self.folder_action_popup_mode_stage = int(stage_value)

        if int(stage_value) == 1:
            for widget in stage1_widgets:
                if widget is not None:
                    widget.show()
            if stage2_section is not None:
                stage2_section.hide()
            if stage3_section is not None:
                stage3_section.hide()
        ...
        if int(stage_value) == 2:
            if stage2_section is not None:
                stage2_section.show()
            if stage3_section is not None:
                stage3_section.hide()
            if title_label is not None:
                title_label.setText(f"Sort Assets\n{disk_name}")
            if hint_label is not None:
                hint_label.setText("Click the button to run Sort for this stage-2 folder.")
            if stage2_button is not None:
                stage2_button.setText("Run Sort")
            return
        ...
        if stage3_section is not None:
            stage3_section.show()
        if title_label is not None:
            title_label.setText(f"Stage 3 Assets\n{disk_name}")
        if hint_label is not None:
            hint_label.setText(
                "Two independent panels: Bulk Upload and PDF.\n"
                "Drag the divider between panels to resize/move their space."
            )

    def _run_stage2_sort_from_asset(self) -> None:
        if self._resolve_folder_action_stage() != 2:
            self._show_message_box_topmost_non_modal(
        ...
        disk_name = str(self.folder_action_anchor_disk_name or "").strip()
        if not disk_name:
            selected_names = self._folder_action_selected_disk_names_for_stage(2)
            if selected_names:
                disk_name = selected_names[0]
        if not disk_name:
            self._show_message_box_topmost_non_modal(
                QMessageBox.Icon.Information,
                "2. Sort",
        ...
        proceed = self._ask_question_topmost_non_modal(
            "2. Sort",
            (
                f"Run sorting for this folder?\n\nFolder: {disk_name}\n\n"
                "Yes = Move files into '<folder>/sorted'\n"
                "No = Copy files into '<folder>/sorted'\n"
                "Cancel = Stop"
            ),
        ...
        if proceed == QMessageBox.Cancel:
            return
        copy_files = proceed == QMessageBox.No

        try:
            results = self._run_blocking_io_task(
                "2. Sort",
                "Sorting stage-2 folder...\nPlease wait.",
                lambda: sort_folders(
                    [folder_path],
            ...
            self._show_user_error_dialog(
                "2. Sort Failed",
                "DAMYComp could not finish the sort operation.",
                possible_causes=[
                    "One or more folders are locked or inaccessible.",
                    "A selected folder does not contain files expected by sorter.",
                ],
                next_steps=[
                    "Close file explorers/editors using these folders and try again.",
                    "Check folder contents and naming, then rerun.",
                ],
                details=str(exc),
            )
            return
        ...
        summary_lines = [
            ...
            f"{'Copied' if copy_files else 'Moved'} files: {total_moved}",
        ]
        if skipped:
    ...
    def _resolve_existing_folder_path_for_open(self, disk_name: str) -> str | None:
    ...
    def _open_stage3_selection_from_asset(self, action_name: str) -> None:
        selected_names = self._folder_action_selected_disk_names_for_stage(3)
        if not selected_names:
            self._show_message_box_topmost_non_modal(
                QMessageBox.Icon.Information,
                action_name,
                "Select one or more folders in '3. Photodeck Upload/PDF Packets' first.",
            )
            return

        found_paths: list[str] = []
        missing_names: list[str] = []
        for disk_name in selected_names:
            folder_path = self._resolve_existing_folder_path_for_open(disk_name)
            if folder_path:
                found_paths.append(folder_path)
            else:
                missing_names.append(disk_name)

        if not found_paths:
            self._show_message_box_topmost_non_modal(
                QMessageBox.Icon.Warning,
                action_name,
                "All selected stage-3 folders are missing on disk.",
            )
            return

        if len(found_paths) > 1:
            confirm = self._ask_question_topmost_non_modal(
                action_name,
                f"Open {len(found_paths)} selected folder(s)?",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.Yes,
            )
        ...
        for folder_path in found_paths:
            try:
                self._open_path_in_system(Path(folder_path))
            except Exception as exc:
                self._show_message_box_topmost_non_modal(
                    QMessageBox.Icon.Warning,
                    action_name,
        ...
        if missing_names:
            ...
            self._show_message_box_topmost_non_modal(
                QMessageBox.Icon.Warning,
                action_name,
                f"Skipped missing folder(s): {preview}",
            )

    def _run_stage3_bulk_upload_from_asset(self) -> None:
        self._open_stage3_selection_from_asset("Bulk Upload")

    def _run_stage3_pdf_from_asset(self) -> None:
        self._open_stage3_selection_from_asset("PDF")
    ...
    def _show_folder_action_popup_for_item(self, lw: DragListWidget, item: QListWidgetItem) -> None:
        ...
        stage_value = self._resolve_folder_action_stage(lw)
        if not self._stage_supports_folder_assets(stage_value):
            self._hide_folder_action_popup()
            return

        click_key = (id(lw), item_id)
        if self._folder_action_toggle_suppress_key == click_key:
            self._folder_action_toggle_suppress_key = None
            return
        ...
        if (
            ...
            self._folder_action_toggle_suppress_key = click_key
            QTimer.singleShot(
                0,
                lambda key=click_key: self._clear_folder_action_toggle_suppress_key(key),
            )
            self._hide_folder_action_popup(clear_selection=True)
            return
        ...
        selected_count = len([it for it in lw.selectedItems() if it is not None])
        self._set_folder_action_popup_mode(stage_value, disk_name, selected_count=max(1, selected_count))
        if int(stage_value) == 1:
    ...
    def _hide_folder_action_popup(self, *, clear_selection: bool = False) -> None:
        ...
        self.folder_action_popup_mode_stage = None
        self._folder_action_show_orders_form_slot2 = False
        self._folder_action_show_orders_form_slot3 = False
        self._folder_action_show_orders_form_slot4 = False
        self._folder_action_show_pdf_slot2 = False
    ...
    def _on_folder_action_expand_second_card(self, asset_kind: str) -> None:
        ...
        if kind == "pdf" and expanded:
            # Prevent accidental file-picker popups when the layout shifts after clicking "+".
    ...
    def _collect_linked_pdf_paths_for_qr(self, db_item, *, preferred_first: Path | None = None) -> list[Path]:
    ...
    def _on_folder_action_qr_card_clicked(self, qr_kind: str) -> None:
        ...
        if target is None:
            return
        _item_id, db_item, _folder_dir = target
    ...
    def _safe_list_stage(self, stage: int):
        try:
            return self._run_db_operation_with_retry(
                ...
                lambda: self.db.list_by_stage(stage, domain=WORKFLOW_DOMAIN_PREPAID),
                attempts=4,
    ...
    def _safe_list_all_by_stage(self) -> dict[int, list]:
        ...
        try:
            all_rows = self._run_db_operation_with_retry(
                "list_all(board_init)",
                lambda: self.db.list_all(domain=WORKFLOW_DOMAIN_PREPAID),
                attempts=4,
                initial_delay_seconds=0.15,
            )
            for row in all_rows:
    ...
    def _build_stage1_asset_scan_payload(
        ...
        payload: list[tuple[int, str, str, str, str, str, str, str, str, str, str, str]] = []
    ...
    def make_toggle_column_visibility(self, selected_column):
        def toggle(event):
            pinned_column = self.upcoming_column_widget
            if self.expanded_column == selected_column:
                ...
                for col, _ in self.column_widgets:
                    if pinned_column is not None and col is pinned_column:
                        col.setVisible(True)
                    else:
    ...
    def _pending_precheck_counts(self) -> tuple[int, int, int] | None:
        ...
        disk_only_count = len(disk_names - db_names)
    ...
    def _on_refresh_clicked(self) -> None:
        target = (self.current_workspace or "prepaid").strip().lower()
        if target in {"proofing", "yearbook"}:
            self._run_embedded_workspace_refresh(target)
            return
        self._refresh_retry_remaining = 5
        self._refresh_completion_pending = True
        self._run_refresh_flow(from_auto_retry=False)

    def _run_embedded_workspace_refresh(self, target: str) -> None:
        workspace_key = (target or "").strip().lower()
        if workspace_key not in {"proofing", "yearbook"}:
            return
        ...
        if workspace_key == "proofing":
            self._attempt_proofing_embed_load()
        else:
            self._attempt_yearbook_embed_load()

        widget = self._active_workspace_widget(workspace_key)
        if widget is None:
            self._show_info_message_dialog(
                "Refresh",
                f"{self._workspace_display_name(workspace_key)} UI is still loading.\nPlease try refresh again in a moment.",
                min_width=460,
                min_height=200,
            )
            return

        busy, busy_label = self._embedded_workspace_busy_state(workspace_key)
        if busy:
            running_label = (busy_label or self._workspace_display_name(workspace_key)).strip()
            self._show_info_message_dialog(
                "Program Running",
                f"{running_label} is running.\nPlease wait, then refresh again.",
                min_width=440,
                min_height=200,
            )
            return
        ...
        added_by_domain: dict[str, int] = {
            WORKFLOW_DOMAIN_PREPAID: 0,
            WORKFLOW_DOMAIN_PROOFING: 0,
            WORKFLOW_DOMAIN_YEARBOOK: 0,
        }
        deleted_stale_rows = 0

        disk_names = self._list_sync_candidate_folders_from_dir(
            self._domain_base_dir_for_refresh(workspace_key)
        )
        try:
            all_domain_db_names = set(
                self._run_db_operation_with_retry(
                    f"list_disk_names(refresh:{workspace_key}:all-domains)",
                    self._list_disk_names_all_domains_for_precheck,
                    attempts=5,
                    initial_delay_seconds=0.2,
                )
            )
            ...
            self._report_db_issue(
                "Refresh Failed",
                f"Could not read workflow DB rows for {self._workspace_display_name(workspace_key)} refresh.",
                e,
            )
            return

        disk_only = sorted(disk_names - all_domain_db_names, key=str.lower)
        if disk_only:
            ...
            for folder_name, domain, stage in mapped_rows:
                try:
                    new_id = int(
                        self._upsert_from_disk_name_with_retry(
                            folder_name,
                            int(stage),
                            domain=domain,
                        )
                    )
                    self.newly_added_item_ids.add(new_id)
                    added_item_ids.add(new_id)
                    added_to_db += 1
                    domain_key = (domain or WORKFLOW_DOMAIN_PREPAID).strip().lower() or WORKFLOW_DOMAIN_PREPAID
                    if domain_key not in added_by_domain:
                        domain_key = WORKFLOW_DOMAIN_PREPAID
                    added_by_domain[domain_key] += 1
                except Exception as e:
                    upsert_errors.append(f"{folder_name} [{domain} stage {stage}]: {e}")

            if upsert_errors:
                preview = "\n".join(upsert_errors[:60])
                if len(upsert_errors) > 60:
                    preview += f"\n...and {len(upsert_errors) - 60} more"
                self._show_info_text_dialog(
                    "Refresh Summary",
                    "Some folders could not be added to the workflow DB:\n\n" + preview,
                    min_width=820,
                    min_height=460,
                )

        stale_rows = self._collect_non_prepaid_missing_rows(domains=(workspace_key,))
        if stale_rows:
            action, selected_ids = self._ask_non_prepaid_missing_rows_qt(
                stale_rows,
                dialog_title=f"Missing {self._workspace_display_name(workspace_key)} Folders",
                intro_text=(
                    f"These rows are in {self._workspace_display_name(workspace_key)} workflow DB, "
                    "but their target folders are missing on disk.\n\n"
                    "Review and delete stale rows now, or skip and keep them for later."
                ),
            )
            if action == "back":
                return
            if action == "delete_selected":
                selected_id_set = {int(v) for v in selected_ids if int(v) > 0}
                if selected_id_set:
                    confirm_delete = self._confirm_dangerous_action(
                        title="Confirm DB Delete",
                        happened=f"Delete {len(selected_id_set)} stale {self._workspace_display_name(workspace_key)} row(s)?",
                        impacts=[
                    ...
                    if confirm_delete:
                        ...
                        deleted_count = 0
                        for item_id in sorted(selected_id_set):
                            try:
                                self.db.delete_item(int(item_id))
                                deleted_count += 1
                            except Exception as e:
                                delete_errors.append(f"id={item_id}: {e}")
                        if delete_errors:
                            self._show_info_text_dialog(
                                "Refresh Summary",
                                "Some stale rows could not be deleted:\n\n"
                                + "\n".join(delete_errors[:40])
                                + (f"\n...and {len(delete_errors) - 40} more" if len(delete_errors) > 40 else ""),
                                min_width=820,
                                min_height=460,
                            )
                        if deleted_count > 0:
                            deleted_stale_rows += int(deleted_count)

        try:
            if added_item_ids and hasattr(widget, "ingest_external_markers"):
                try:
                    widget.ingest_external_markers(new_ids=set(added_item_ids))
                except Exception:
                    pass
            if hasattr(widget, "reload_ui"):
                widget.reload_ui()
        except Exception as e:
            self._show_user_error_dialog(
                "Refresh Failed",
                f"Could not refresh {self._workspace_display_name(workspace_key)} view.",
                possible_causes=[
                    "The embedded workspace failed while reloading.",
                    "The workflow DB connection dropped during the request.",
                ],
                next_steps=[
                    "Wait a moment and try refresh again.",
                    "If the issue continues, check DB connectivity.",
                ],
                details=str(e),
            )
            return

        self._sync_global_search_to_workspace(workspace_key)
        status_suffix: list[str] = []
        if added_to_db > 0:
            status_suffix.append(
                "added "
                f"{added_to_db} rows "
                f"(Prepaid={added_by_domain[WORKFLOW_DOMAIN_PREPAID]}, "
                f"Proofing={added_by_domain[WORKFLOW_DOMAIN_PROOFING]}, "
                f"YearBook={added_by_domain[WORKFLOW_DOMAIN_YEARBOOK]})"
            )
        if deleted_stale_rows > 0:
            status_suffix.append(f"removed {deleted_stale_rows} stale rows")

        if status_suffix:
            self._set_completion_status(
                f"Refresh complete. {self._workspace_display_name(workspace_key)} board is up to date ({'; '.join(status_suffix)})."
            )
        else:
            self._set_completion_status(
                f"Refresh complete. {self._workspace_display_name(workspace_key)} board is up to date."
            )
    ...
    def _delete_db_item_by_disk_name(
        self,
        disk_name: str,
        *,
        ...
        try:
            item = self.db.get_item_by_disk_name(disk_name, domain=domain, stage=stage)
            if item:
    ...
    def _rename_db_item_by_disk_name(
        self,
        old_disk_name: str,
        new_disk_name: str,
        *,
        ...
        if not new_disk_name or old_disk_name == new_disk_name:
            return
        try:
            item = self.db.get_item_by_disk_name(old_disk_name, domain=domain, stage=stage)
            if item:
    ...
    def perform_search(self, text: str):
        workspace = (self.current_workspace or "prepaid").strip().lower()
        if workspace in {"proofing", "yearbook"}:
            self._perform_upcoming_only_search(text)
            self._perform_embedded_workspace_search(workspace, text)
            return
        self._perform_prepaid_search(text)

    def _perform_prepaid_search(self, text: str) -> None:
        self._hide_folder_action_popup()
        needle = (text or "").strip().lower()
        pinned_column = self.upcoming_column_widget
        ...
        for col_widget, lw in self.column_widgets:
            ...
            if self.edit_widgets and lw == self.edit_widgets[0]:
                ...
                for i in range(lw_edit.count()):
                    ...
                    if match:
                        any_visible = True
                col_widget.setVisible((col_widget is pinned_column) or any_visible or not needle)
            else:
        ...
        for stage_def in STAGES:
            for prefix in stage_def.prefixes:
                if s.startswith(prefix):
                    return True
        # Accept numbered non-Prepaid workflow prefixes (Proofing/YearBook),
        # but avoid broad false positives like template folders.
    ...
    def _list_disk_names_all_domains_for_precheck(self) -> set[str]:
        names: set[str] = set()
        for domain in (WORKFLOW_DOMAIN_PREPAID, WORKFLOW_DOMAIN_PROOFING, WORKFLOW_DOMAIN_YEARBOOK):
            names |= set(self.db.list_disk_names(domain=domain))
        return names

    def _format_stage_label(self, stage_value: int, label: str) -> str:
        text = str(label or "").strip() or f"Stage {int(stage_value)}"
        if re.match(rf"^\s*{int(stage_value)}\.\s+", text):
            return text
        return f"{int(stage_value)}. {text}"

    def _domain_stage_options(self, domain: str) -> list[tuple[int, str]]:
        domain_norm = (domain or WORKFLOW_DOMAIN_PREPAID).strip().lower() or WORKFLOW_DOMAIN_PREPAID
        if domain_norm == WORKFLOW_DOMAIN_PROOFING:
            return [
                (1, "1. Make Proofs"),
                (2, "2. Sort"),
                (3, "3. Photodeck Upload/PDF Packets"),
                (4, "4. Email School"),
                (5, "5. Finished"),
                (6, "6. Import Photodeck"),
                (7, "7. Edit"),
                (8, "8. Print"),
                (9, "9. Package"),
                (10, "10. Deliver"),
            ]
        if domain_norm == WORKFLOW_DOMAIN_YEARBOOK:
            return [
                (1, "1. Make Proofs"),
                (2, "2. Sort"),
                (3, "3. Photodeck Upload/PDF Packets"),
        ...
        return [
            (int(s.stage), self._format_stage_label(int(s.stage), str(s.label)))
            for s in STAGES
        ]

    def _infer_stage_for_domain(
        self,
        folder_name: str,
        domain: str,
        *,
        ...
        options = self._domain_stage_options(domain)
        valid_stage_ids = {int(stage) for stage, _label in options}
        if current_stage is not None:
            try:
                current_value = int(current_stage)
            except Exception:
                current_value = 0
            if current_value in valid_stage_ids:
                return current_value

        domain_norm = (domain or WORKFLOW_DOMAIN_PREPAID).strip().lower() or WORKFLOW_DOMAIN_PREPAID
        guessed_stage = 1
        if domain_norm == WORKFLOW_DOMAIN_PREPAID:
            guessed_stage = int(detect_stage_from_disk_name(folder_name))
        else:
            match = re.match(r"^\s*(\d+)\.\s+", str(folder_name or "").strip())
            if match:
                try:
                    guessed_stage = int(match.group(1))
                except Exception:
                    guessed_stage = 1
        if guessed_stage in valid_stage_ids:
            return guessed_stage
        if not options:
            return 1
        return int(options[0][0])

    def _domain_base_dir_for_refresh(self, domain: str) -> str:
        domain_norm = (domain or WORKFLOW_DOMAIN_PREPAID).strip().lower() or WORKFLOW_DOMAIN_PREPAID
        if domain_norm == WORKFLOW_DOMAIN_PROOFING:
            return (self._proofing_embed_base_dir or self.base_dir or r"T:\DAMY").strip() or (self.base_dir or r"T:\DAMY")
        if domain_norm == WORKFLOW_DOMAIN_YEARBOOK:
            return (self._yearbook_embed_base_dir or self.base_dir or r"T:\DAMY").strip() or (self.base_dir or r"T:\DAMY")
        return (self.base_dir or r"T:\DAMY").strip() or r"T:\DAMY"

    def _domain_runtime_workspace_root_for_refresh(self, domain: str) -> str:
        domain_key = re.sub(r"[^a-z0-9_-]+", "_", str(domain or "").strip().lower() or WORKFLOW_DOMAIN_PREPAID)
        base_dir = self._domain_base_dir_for_refresh(domain)
        return os.path.join(base_dir, "_workflow_runtime", domain_key)

    def _expected_paths_for_row(self, *, domain: str, stage: int, disk_name: str) -> list[str]:
        domain_norm = (domain or WORKFLOW_DOMAIN_PREPAID).strip().lower() or WORKFLOW_DOMAIN_PREPAID
        folder_name = (disk_name or "").strip()
        if not folder_name:
            return []
        if domain_norm == WORKFLOW_DOMAIN_PREPAID:
            paths = [os.path.join(self.base_dir, folder_name)]
            source_dir = (self.source_base_dir or "").strip()
            if source_dir and os.path.normcase(source_dir) != os.path.normcase(self.base_dir):
                paths.append(os.path.join(source_dir, folder_name))
            return paths
        if int(stage) == 7:
            return [os.path.join(self._domain_runtime_workspace_root_for_refresh(domain_norm), "send_to_edit", folder_name)]
        if int(stage) == 8:
            return [os.path.join(self._domain_runtime_workspace_root_for_refresh(domain_norm), "print", folder_name)]
        return [os.path.join(self._domain_base_dir_for_refresh(domain_norm), folder_name)]

    def _collect_non_prepaid_missing_rows(
        self,
        *,
        domains: tuple[str, ...] | None = None,
    ) -> list[dict[str, object]]:
        stale_rows: list[dict[str, object]] = []
        domain_list = domains or (WORKFLOW_DOMAIN_PROOFING, WORKFLOW_DOMAIN_YEARBOOK)
        for domain in domain_list:
            try:
                rows = self._run_db_operation_with_retry(
                    f"list_by_domain(precheck:{domain})",
                    lambda d=domain: self.db.list_by_domain(d),
                    attempts=4,
                    initial_delay_seconds=0.15,
                )
            except Exception as e:
                self._report_db_issue(
                    ...
                    f"Could not read {domain} rows from workflow DB.",
                    e,
                    suppress_transient_popup=True,
                )
                return []
            for row in rows:
                ...
                try:
                    ...
                    stage = 0
                expected_paths = self._expected_paths_for_row(
                    domain=domain,
                    stage=stage,
                    disk_name=disk_name,
                )
                if not expected_paths:
                    continue
                if any(os.path.isdir(path) for path in expected_paths):
                    continue
                stale_rows.append(
                    {
    ...
    def _ask_disk_only_map_qt(self, folder_names: list[str]) -> tuple[str, list[tuple[str, str, int]]]:
        dlg = QDialog(self)
        ...
        domains = [
            (WORKFLOW_DOMAIN_PREPAID, self._workspace_display_name(WORKFLOW_DOMAIN_PREPAID)),
            (WORKFLOW_DOMAIN_PROOFING, self._workspace_display_name(WORKFLOW_DOMAIN_PROOFING)),
            (WORKFLOW_DOMAIN_YEARBOOK, self._workspace_display_name(WORKFLOW_DOMAIN_YEARBOOK)),
        ]
        names = list(folder_names)
        states_stage: dict[str, int] = {}
        states_domain: dict[str, str] = {}
        for name in names:
            states_domain[name] = WORKFLOW_DOMAIN_PREPAID
            states_stage[name] = int(
                self._infer_stage_for_domain(
                    name,
                    WORKFLOW_DOMAIN_PREPAID,
                )
            )
        ...
        layout.addWidget(
            QLabel(
                ...
                "Review the folder list below. Choose workspace + stage for each row before applying."
            )
        )
        ...
        table.setHorizontalHeaderLabels(["Folder", "Workspace", "Stage"])
        ...
        def _refresh_summary() -> None:
            selected_count = sum(1 for n in names if int(states_stage[n]) > 0)
            domain_counts = {
                WORKFLOW_DOMAIN_PREPAID: 0,
                WORKFLOW_DOMAIN_PROOFING: 0,
                WORKFLOW_DOMAIN_YEARBOOK: 0,
            ...
            for n in names:
                stage_value = int(states_stage[n])
                if stage_value <= 0:
                    continue
                domain_key = (
                    str(states_domain.get(n) or WORKFLOW_DOMAIN_PREPAID).strip().lower()
                    or WORKFLOW_DOMAIN_PREPAID
                )
                if domain_key not in domain_counts:
                    domain_key = WORKFLOW_DOMAIN_PREPAID
                domain_counts[domain_key] += 1
            summary_label.setText(
                "Selected to add into DB: "
                f"{selected_count} / {len(names)}    "
                f"Prepaid: {domain_counts[WORKFLOW_DOMAIN_PREPAID]}    "
                f"Proofing: {domain_counts[WORKFLOW_DOMAIN_PROOFING]}    "
                f"YearBook: {domain_counts[WORKFLOW_DOMAIN_YEARBOOK]}"
            )

        def _populate_stage_combo(
            combo: QComboBox,
            *,
            folder_name: str,
            domain_key: str,
            ...
            normalized_stage = int(target_stage)
            if normalized_stage > 0:
                normalized_stage = int(
                    self._infer_stage_for_domain(
                        folder_name,
                        domain_key,
            ...
            try:
                combo.clear()
                combo.addItem("Ignore", 0)
                for stage_value, stage_label in self._domain_stage_options(domain_key):
                    combo.addItem(stage_label, int(stage_value))
                stage_idx = 0
                if normalized_stage > 0:
                    for i in range(combo.count()):
                        if int(combo.itemData(i) or 0) == int(normalized_stage):
                            stage_idx = i
                            break
                combo.setCurrentIndex(stage_idx)
            finally:
        ...
        def _refresh_table() -> None:
            ...
            try:
                ...
                for name in names:
                    ...
                    domain_combo = NoWheelComboBox()
                    for domain_value, domain_label in domains:
                        domain_combo.addItem(domain_label, domain_value)
                    target_domain = (
                        str(states_domain.get(name) or WORKFLOW_DOMAIN_PREPAID).strip().lower()
                        or WORKFLOW_DOMAIN_PREPAID
                    )
                    domain_idx = 0
                    for i in range(domain_combo.count()):
                        if str(domain_combo.itemData(i) or "").strip().lower() == target_domain:
                            domain_idx = i
                            break
                    domain_combo.setCurrentIndex(domain_idx)
                    table.setCellWidget(row, 1, domain_combo)

                    stage_combo = NoWheelComboBox()
                    normalized_stage = _populate_stage_combo(
                        stage_combo,
                        folder_name=name,
                        domain_key=target_domain,
                        target_stage=int(states_stage.get(name, 0) or 0),
                    )
                    states_stage[name] = int(normalized_stage)
                    table.setCellWidget(row, 2, stage_combo)

                    def _on_domain_changed(_i, n=name, dc=domain_combo, sc=stage_combo) -> None:
                        domain_value = (
                            str(dc.currentData() or WORKFLOW_DOMAIN_PREPAID).strip().lower()
                            or WORKFLOW_DOMAIN_PREPAID
                        )
                        states_domain[n] = domain_value
                        current_stage = int(sc.currentData() or states_stage.get(n, 0) or 0)
                        states_stage[n] = int(
                            _populate_stage_combo(
                                sc,
                                folder_name=n,
                    ...
                    domain_combo.currentIndexChanged.connect(_on_domain_changed)

                    stage_combo.currentIndexChanged.connect(
                        lambda _i, n=name, sc=stage_combo: (
                            states_stage.__setitem__(n, int(sc.currentData() or 0)),
                            _refresh_summary(),
                        )
                    )
            finally:
        ...
        domain_all_combo = NoWheelComboBox()
        for domain_value, domain_label in domains:
            domain_all_combo.addItem(domain_label, domain_value)
        btn_set_domain_all = QPushButton("Set Workspace For All")
        tools_row.addWidget(btn_select_all_ok)
        tools_row.addWidget(btn_ignore_all)
        tools_row.addWidget(domain_all_combo)
        tools_row.addWidget(btn_set_domain_all)
        ...
        action_help = QLabel(
            "Apply Selected: add selected rows into workflow DB using chosen Workspace + Stage.\n"
        ...
        def _set_all_decision(is_ok: bool) -> None:
            for n in names:
                if is_ok:
                    if int(states_stage[n]) <= 0:
                        states_stage[n] = int(
                            self._infer_stage_for_domain(
                                n,
                                states_domain.get(n) or WORKFLOW_DOMAIN_PREPAID,
                            )
                        )
                else:
                    states_stage[n] = 0
        ...
        def _set_workspace_for_all() -> None:
            target_domain = (
                str(domain_all_combo.currentData() or WORKFLOW_DOMAIN_PREPAID).strip().lower()
                or WORKFLOW_DOMAIN_PREPAID
            )
            for n in names:
                states_domain[n] = target_domain
                if int(states_stage[n]) > 0:
                    states_stage[n] = int(
                        self._infer_stage_for_domain(
                            n,
                            target_domain,
                            current_stage=int(states_stage[n]),
                        )
                    )
            _refresh_table()

        def _apply() -> None:
            mapped_rows: list[tuple[str, str, int]] = []
            for n in names:
                stage_value = int(states_stage[n])
                if stage_value <= 0:
                    continue
                ...
                mapped_rows.append((n, domain_value, stage_value))
            result["action"] = "apply"
        ...
        btn_set_domain_all.clicked.connect(_set_workspace_for_all)
    ...
    def _ask_non_prepaid_missing_rows_qt(
        self,
        rows: list[dict[str, object]],
        *,
        dialog_title: str = "Missing Proofing/YearBook Folders",
        intro_text: str | None = None,
    ) -> tuple[str, list[int]]:
        dlg = QDialog(self)
        dlg.setWindowTitle(dialog_title)
        dlg.setModal(True)
        dlg.setMinimumSize(1120, 620)
        dlg.setStyleSheet(
            "QDialog { background: #20252e; color: #e8ecf1; }"
            "QLabel { color: #dbe3ee; }"
            "QLineEdit { min-height: 34px; }"
        ...
        intro = (
            intro_text
            or (
                "These rows are in Proofing/YearBook workflow DB, but their target folders are missing on disk.\n\n"
                "Review and delete stale rows now, or skip and keep them for later."
            )
        )
        ...
        search_input = QLineEdit()
        search_input.setPlaceholderText("Search workspace, folder name, or expected path...")
        layout.addWidget(search_input)

        table = QTableWidget(0, 5)
        table.setHorizontalHeaderLabels(["Select", "Workspace", "Stage", "Folder", "Expected Folder Path"])
        table.verticalHeader().setVisible(False)
        table.setSelectionMode(QAbstractItemView.SelectionMode.NoSelection)
        table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        table.horizontalHeader().setSectionResizeMode(1, table.horizontalHeader().ResizeMode.ResizeToContents)
        table.horizontalHeader().setSectionResizeMode(2, table.horizontalHeader().ResizeMode.ResizeToContents)
        table.horizontalHeader().setSectionResizeMode(3, table.horizontalHeader().ResizeMode.ResizeToContents)
        table.horizontalHeader().setSectionResizeMode(4, table.horizontalHeader().ResizeMode.Stretch)
        layout.addWidget(table)

        state_by_id: dict[int, Qt.CheckState] = {
            int(row.get("id", 0) or 0): Qt.CheckState.Checked
            for row in rows
        ...
        def _refresh_count() -> None:
            selected_count = sum(1 for item_id in state_by_id if state_by_id[item_id] == Qt.CheckState.Checked)
            selected_count_label.setText(f"Selected stale rows: {selected_count} / {len(state_by_id)}")
        ...
        def _refresh_table() -> None:
            ...
            try:
                ...
                for row in rows:
                    ...
                    disk_name = str(row.get("disk_name", "") or "").strip()
                    expected_paths = [str(p) for p in (row.get("expected_paths", []) or []) if str(p).strip()]
                    expected_text = "\n".join(expected_paths)
                    haystack = f"{domain} {stage} {disk_name} {expected_text}".lower()
                    ...
                    select_item.setCheckState(state_by_id.get(item_id, Qt.CheckState.Checked))
                    select_item.setData(Qt.ItemDataRole.UserRole, item_id)
                    table.setItem(row_idx, 0, select_item)
                    table.setItem(row_idx, 1, QTableWidgetItem(domain.title()))
                    table.setItem(row_idx, 2, QTableWidgetItem(str(stage)))
                    table.setItem(row_idx, 3, QTableWidgetItem(disk_name))
                    path_item = QTableWidgetItem(expected_text)
                    path_item.setToolTip(expected_text)
                    table.setItem(row_idx, 4, path_item)
            finally:
                rebuilding["value"] = False
            _refresh_count()
        ...
        def _on_item_changed(item: QTableWidgetItem) -> None:
            ...
            if item_id <= 0:
                return
            state_by_id[item_id] = item.checkState()
            _refresh_count()

        def _set_all(checked: bool) -> None:
            state = Qt.CheckState.Checked if checked else Qt.CheckState.Unchecked
            for item_id in list(state_by_id.keys()):
                state_by_id[item_id] = state
            _refresh_table()
        ...
        result = {"action": "back", "selected_ids": []}
        action_row = QHBoxLayout()
        btn_delete = QPushButton("Delete Selected")
        btn_delete.setStyleSheet(
            "QPushButton { min-height: 34px; padding: 6px 12px; border-radius: 8px; "
        ...
        btn_skip = QPushButton("Skip For Now")
        btn_cancel = QPushButton("Cancel Refresh")
        action_row.addWidget(btn_delete)
        action_row.addWidget(btn_skip)
        ...
        action_help = QLabel(
            "Delete Selected: remove stale rows from workflow DB only.\n"
            "Skip For Now: keep rows and continue refresh."
        )
        action_help.setWordWrap(True)
        layout.addWidget(action_help)

        def _choose_delete() -> None:
            selected_ids = [item_id for item_id, state in state_by_id.items() if state == Qt.CheckState.Checked]
            result["action"] = "delete_selected"
            result["selected_ids"] = selected_ids
            dlg.accept()

        def _choose_skip() -> None:
            result["action"] = "skip_all"
            result["selected_ids"] = []
            dlg.accept()

        def _choose_cancel() -> None:
            result["action"] = "back"
            result["selected_ids"] = []
            dlg.reject()
        ...
        btn_skip.clicked.connect(_choose_skip)
        btn_cancel.clicked.connect(_choose_cancel)

        self._attach_responsive_font_scaling(
            dlg,
            text_widgets=[search_input, table, selected_count_label],
            button_widgets=[btn_select_all, btn_clear_all, btn_delete, btn_skip, btn_cancel],
            base_width=1120,
        )

        self._show_dialog_topmost_non_modal(dlg)
        selected_ids = [int(v) for v in result["selected_ids"] if int(v) > 0]
        return str(result["action"]), selected_ids
    ...
    def _run_pre_import_sync_checks(self) -> bool:
        try:
            prepaid_db_names = set(
                self._run_db_operation_with_retry(
                    "list_disk_names(precheck:prepaid)",
                    lambda: self.db.list_disk_names(domain=WORKFLOW_DOMAIN_PREPAID),
                    attempts=5,
                    initial_delay_seconds=0.2,
                )
            )
        ...
        try:
            all_domain_db_names = set(
                self._run_db_operation_with_retry(
                    "list_disk_names(precheck:all-domains)",
                    self._list_disk_names_all_domains_for_precheck,
                    attempts=5,
                    initial_delay_seconds=0.2,
                )
            ...
            self._report_db_issue(
                "Pre-Import Check Failed",
                "Could not read DB rows across all workspaces.",
                e,
                suppress_transient_popup=True,
            )
            return False
        ...
        disk_names = self._list_sync_candidate_folders()
        db_candidate_names = {name for name in prepaid_db_names if self._looks_like_workflow_folder_name(name)}
        disk_only = sorted(disk_names - all_domain_db_names, key=str.lower)
        ...
        if disk_only:
            ...
            for folder_name, domain, stage in mapped_rows:
                try:
                    new_id = self._upsert_from_disk_name_with_retry(
                        folder_name,
                        stage,
                        domain=domain,
                    )
                    ...
                    detail = f"{folder_name} [{domain} stage {stage}]: {e}"
        ...
        if disk_only:
            summary_lines.append(
                "  by workspace: "
                f"Prepaid={added_by_domain.get(WORKFLOW_DOMAIN_PREPAID, 0)}, "
                f"Proofing={added_by_domain.get(WORKFLOW_DOMAIN_PROOFING, 0)}, "
                f"YearBook={added_by_domain.get(WORKFLOW_DOMAIN_YEARBOOK, 0)}"
            )
    ...
    def _upsert_from_disk_name_with_retry(
        self,
        folder_name: str,
        stage: int,
        *,
            ...
            try:
                ...
                if attempt < attempts and self._is_transient_db_error(exc):
                    _append_ui_runtime_log(
                        ...
                        f"domain={domain_norm} stage={target_stage} "
                        f"sqlstate={sqlstate or '(none)'} err={exc}"
                    )
                    time.sleep(delay)
