...
def run_migrations(conninfo: str, *, statement_timeout_seconds: int = 30) -> None:
    with psycopg.connect(conninfo) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT name FROM schema_migrations WHERE version=1;")
            first_migration = cur.fetchone()
            if first_migration and first_migration[0] != "create_v2_workflow_schema":
                raise RuntimeError(
                    "This database was initialized with the old workflow schema. "
                    "Create/use damy_workflow_v2 and migrate old data instead of running v2 migrations in place."
                )
            cur.execute("SELECT to_regclass('workflow_items'), to_regclass('workflow_runs');")
            workflow_items_table, workflow_runs_table = cur.fetchone()
            if workflow_items_table is not None and workflow_runs_table is None:
                raise RuntimeError(
                    "This database contains the old workflow_items table and is not a v2 database. "
                    "Create/use damy_workflow_v2 and run folder_manager.migrate_workflow_v2."
                )
