...
from PySide6.QtGui import QBrush, QColor, QDrag, QDragEnterEvent, QDropEvent, QKeySequence, QPainter, QPalette
...
class DragListWidget(QListWidget):
    ...
    def _resolve_open_path(self, item: QListWidgetItem) -> str:
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        if self._can_accept_workflow_drop(event.source()):
    ...
    @staticmethod
    def _stage_drag_group(stage: object) -> str:
        try:
            stage_int = int(stage or 0)
        except Exception:
            return ""
        if 1 <= stage_int <= 5:
            return "workflow"
    ...
    def _can_accept_workflow_drop(self, source: object) -> bool:
        if not isinstance(source, DragListWidget) or source is self:
            return False
        source_group = self._stage_drag_group(getattr(source, "workflow_stage", None))
        target_group = self._stage_drag_group(self.workflow_stage)
    ...
    def dropEvent(self, event: QDropEvent):
        source = event.source()
        if not self._can_accept_workflow_drop(source):
            return
    ...
    def open_folder(self, item: QListWidgetItem) -> None:
        path = self._resolve_open_path(item)
        if not os.path.exists(path):
            QMessageBox.warning(self, "Open Path Failed", f"Path does not exist:\n{path}")
            return
        try:
            self._open_path_with_system(path)
        except Exception as e:
            QMessageBox.warning(self, "Open Path Failed", f"Could not open:\n{path}\n{e}")

    def open_selected_paths(self) -> None:
        items = [item for item in self.selectedItems() if item is not None]
        if not items and self.currentItem() is not None:
            items = [self.currentItem()]
        if not items:
            QMessageBox.information(self, "Open Selected", "No items selected.")
            return
        ...
        if len(items) > 1:
            confirm = QMessageBox.question(
                self,
                "Open Selected",
                f"Open {len(items)} selected path(s) with the system default app?",
        ...
        if missing or failed:
            ...
            if failed:
                parts.append("Failed:\n" + "\n".join(failed[:10]))
            QMessageBox.warning(self, "Open Selected", "\n\n".join(parts))
